...
module.exports = {
    cmd: "distance",
    alias: ["dist", "citydist"],
    desc: "Calculate distance between two cities",
    category: "utility",
    async execute(conn, m, { text, args }) {
        if (!text || !text.toLowerCase().includes(' to ')) {
            return m.reply(`🌍 *Distance Calculator*\n\n` +
                `*Usage:* .distance <city1> to <city2>\n` +
                `*Example:* .distance nairobi to dubai\n\n` +
                `*Supported Flags:* 🇰🇪 🇵🇰 🇮🇳 🇦🇪 🇬🇧 🇺🇸 🇸🇦`);
        }

        const parts = text.split(/ to /i);
        const city1 = findCity(parts[0].trim());
        const city2 = findCity(parts[1].trim());

        if (!city1) return m.reply(`❌ City not found: *${parts[0]}*`);
        if (!city2) return m.reply(`❌ City not found: *${parts[1]}*`);

        if (city1.name === city2.name) return m.reply(`😄 Both cities are the same! Distance is 0 km.`);

        const km = haversine(city1.lat, city1.lon, city2.lat, city2.lon);
        const miles = km * 0.621371;

        const responseText = `🌍 *Distance Calculator*\n\n` +
            ...
            `✈️ *Flight time:* ${formatTime(km, 900)}\n` +
            `🚗 *Drive time:* ${formatTime(km, 80)}\n\n` +
            `*POPKID-MD*`;
